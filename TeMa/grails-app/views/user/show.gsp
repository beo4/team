			...
			<div class="span9">
				...
				<dl>
					...
					<g:if test="${userInstance?.language}">
						<dt><g:message code="user.language.label" default="Language" /></dt>
						
							<dd><g:link controller="eventLanguage" action="show" id="${userInstance?.language?.id}">${userInstance?.language?.encodeAsHTML()}</g:link></dd>
						
					</g:if>
				
					<g:if test="${userInstance?.event}">
						<dt><g:message code="user.event.label" default="Event" /></dt>
						
							<dd><g:link controller="event" action="show" id="${userInstance?.event?.id}">${userInstance?.event?.encodeAsHTML()}</g:link></dd>
